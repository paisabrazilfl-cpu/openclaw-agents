  ...
  return (
    <div className="min-h-[100dvh] bg-ink-950">
      <header className="sticky top-0 z-10 flex items-center justify-between gap-3 border-b border-ink-700 bg-ink-900/95 px-4 py-3 backdrop-blur sm:px-6">
        ...
        <Link
          href="/"
          className="shrink-0 rounded-md border border-ink-700 px-3 py-1.5 text-xs text-slate-300 hover:bg-ink-800"
        >
          ← Console
        </Link>
      </header>
      ...
      <main className="mx-auto max-w-3xl space-y-6 px-4 py-6 sm:px-6">
        ...
        <section className="rounded-xl border border-ink-700 bg-ink-900">
          <div className="flex items-center justify-between border-b border-ink-700 px-4 py-3">
            ...
            {data && (
              <span className="rounded-full bg-ink-700 px-2.5 py-0.5 text-[11px] font-medium text-slate-300">
          ...
          <ul className="divide-y divide-ink-800">
            {data?.integrations.map((i) => (
              <li key={i.name} className="flex items-center gap-3 px-4 py-2.5">
                ...
                <span className={`rounded px-1.5 py-0.5 text-[10px] font-medium ${CAT_COLOR[i.category] ?? "bg-ink-700 text-slate-400"}`}>
                  {i.category}
                </span>
                <span
                  className={`flex items-center gap-1.5 text-xs font-medium ${i.on ? "text-emerald-400" : "text-slate-600"}`}
                >
                  <span className={`h-2 w-2 rounded-full ${i.on ? "bg-emerald-400" : "bg-slate-600"}`} />
                  {i.on ? "On" : "Off"}
                </span>
              </li>
            ))}
            {!data && <li className="px-4 py-6 text-center text-xs text-slate-500">Loading…</li>}
          </ul>
        </section>
        ...
        <section className="rounded-xl border border-ink-700 bg-ink-900">
          <div className="border-b border-ink-700 px-4 py-3">
            ...
            <p className="mt-0.5 text-[11px] text-slate-500">
              ...
              them anywhere with <code className="text-slate-400">{"{{secret:NAME}}"}</code>.
            </p>
          </div>
          <div className="space-y-2.5 p-4">
            <div className="grid gap-2.5 sm:grid-cols-2">
              ...
              <label className="block">
                <span className="mb-1 block text-[11px] text-slate-500">Description (optional)</span>
                <input
                  value={desc}
            ...
            <label className="block">
              <span className="mb-1 block text-[11px] text-slate-500">Value</span>
              <input
                type="password"
                value={value}
        ...
        <section className="rounded-xl border border-ink-700 bg-ink-900">
          ...
          <ul className="divide-y divide-ink-800">
              ...
              <li key={s.name} className="flex items-center gap-3 px-4 py-2.5">
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    <span className="truncate font-mono text-xs text-slate-200">{s.name}</span>
                    <span className="font-mono text-[11px] text-slate-600">••••••••</span>
                    {s.source === "env" && (
                      <span className="rounded bg-ink-700 px-1.5 py-0.5 text-[10px] text-slate-400">env</span>
                    )}
                  </div>
                  {s.description && <div className="truncate text-[11px] text-slate-500">{s.description}</div>}
                </div>
                <button
                  onClick={() => copyRef(s.name)}
                  title="Copy reference"
                  className="shrink-0 rounded border border-ink-700 px-2 py-1 font-mono text-[10px] text-slate-400 hover:bg-ink-800"
                >
                  {"{{secret:…}}"}
                </button>
                {s.source === "keystore" ? (
            ...
            {data && data.secrets.length === 0 && (
              <li className="px-4 py-6 text-center text-xs text-slate-500">No secrets stored yet.</li>
            )}
          </ul>
        </section>
      ...
      {toast && (
        <div className="fixed bottom-4 left-1/2 z-20 -translate-x-1/2 rounded-lg bg-emerald-600 px-4 py-2 text-xs font-medium text-white shadow-lg">
          {toast}
        </div>
      )}
    </div>
